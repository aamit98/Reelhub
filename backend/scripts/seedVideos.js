...
const seedVideos = async () => {
  try {
    ...
    let defaultUser = await User.findOne({ email: 'demo@aora.com' });
    ...
    if (!defaultUser) {
      defaultUser = new User({
        ...
        email: 'demo@aora.com',
        password: 'demo123456',
        avatar: 'https://ui-avatars.com/api/?name=Demo+Creator&background=random&color=fff&size=200'
      });
    ...
    const existingVideos = await Video.find({ creator: defaultUser._id });
    ...
    const videos = sampleVideos.map(videoData => ({
      ...videoData,
      creator: defaultUser._id
    }));

    await Video.insertMany(videos);
    console.log(`✅ Seeded ${videos.length} sample videos`);
