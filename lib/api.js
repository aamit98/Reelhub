...
// Physical devices need the actual LAN IP address
const getDevHost = () => {
  // Check if we have a debugger host from Expo (works for physical devices via Expo Go)
  // This is available when connected via LAN to Expo dev server
  const debuggerHost = Constants.expoConfig?.hostUri || 
                       Constants.expoGoConfig?.hostUri ||
                       Constants.debuggerHost;
  
  if (debuggerHost) {
    // Extract IP from debugger host (format: "192.168.1.100:8081" or "192.168.1.100")
    let ip = debuggerHost.split(':')[0];
    if (ip && ip !== 'localhost' && ip !== '127.0.0.1') {
      console.log(`[API] Detected device IP from Expo: ${ip}`);
      return `http://${ip}:3000`;
    }
  }
  
  // Also check manifest.extra for manually set IP (useful fallback)
  const manifestExtra = Constants.expoConfig?.extra;
  if (manifestExtra?.localNetworkIP) {
    console.log(`[API] Using manually configured IP: ${manifestExtra.localNetworkIP}`);
    return `http://${manifestExtra.localNetworkIP}:3000`;
  }
  ...
  console.log(`[API] Using localhost (iOS simulator or default)`);
  return "http://localhost:3000";
};

// Helper to get base URL without /api suffix (for constructing file URLs)
export const getBaseUrl = () => {
  const configUrl = Constants.expoConfig?.extra?.apiBaseUrl;
  
  if (configUrl && typeof configUrl === "string" && configUrl.trim() !== "") {
    const url = configUrl.trim();
    // Remove /api if present, we just want the base
...
const apiRequest = async (endpoint, options = {}) => {
  ...
  try {
    ...
    const currentUrl = `${API_BASE_URL}${endpoint}`;
    const hostInfo = getDevHost();
    
    if (error.message.includes('timeout')) {
      throw new Error(`Connection timeout - Check if backend server is running on port 3000\n\nAttempted URL: ${currentUrl}\nExpected host: ${hostInfo}`);
    }
    if (error.message.includes('Failed to fetch') || error.message.includes('Network request failed')) {
      throw new Error(`Cannot connect to server\n\nAttempted URL: ${currentUrl}\nExpected host: ${hostInfo}\n\nTroubleshooting:\n1. Make sure backend is running: cd backend && npm start\n2. For physical devices: Check localNetworkIP in app.json matches your computer's IP\n3. Both device and computer must be on the same WiFi network`);
    }
